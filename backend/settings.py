...
ALLOWED_HOSTS = ['*', '1.15.103.26', 'www.k-irito.online', 'k-irito.online']
...
CSRF_COOKIE_HTTPONLY = False  # 修改为False允许JavaScript读取CSRF令牌
CSRF_USE_SESSIONS = False  # 修改为False使用cookie而不是session
CSRF_COOKIE_SAMESITE = 'Lax'  # 改为Lax以在同一站点内工作
...
SECURE_SSL_REDIRECT = False  # 因为在Nginx中已经设置了重定向
SESSION_COOKIE_SECURE = False  # 修改为False以允许http访问使用会话cookie
CSRF_COOKIE_SECURE = False  # 修改为False以允许http访问使用CSRF cookie

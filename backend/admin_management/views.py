...
def log_system_error(level, message, request=None, exception=None, error_type='BACKEND', stack_trace=None, context=None):
    """
    记录系统错误的工具函数
    ...
    - request: 请求对象
    - exception: 异常对象
    - error_type: 错误类型 (FRONTEND, BACKEND, DATABASE 等)
    - stack_trace: 可选的预格式化堆栈跟踪
    - context: 额外的上下文信息字典
    """
    try:
        # 创建上下文数据
        ...
        # 如果提供了预格式化的堆栈跟踪，添加到上下文
        if stack_trace:
            context_data['stack_trace'] = stack_trace
        
        # 如果没有指定错误类型，尝试推断
        inferred_type = _infer_error_type_from_exception(exception) if exception else None
        if inferred_type and error_type == 'BACKEND':
            error_type = inferred_type
        
        # 如果没有指定错误级别，尝试推断
        inferred_level = _infer_error_level_from_type(error_type, exception) if level is None else None
        if inferred_level:
        ...
        if exception:
            try:
                # 尝试获取完整堆栈跟踪
                if stack_trace:
                    error_log.traceback = stack_trace
                else:
                    # 获取完整的堆栈跟踪，包括嵌套异常
                    tb_parts = []
                    
                    # 处理当前异常
                    tb_parts.append(''.join(traceback.format_exception(
                    ...
                    if hasattr(exception, '__cause__') and exception.__cause__:
                        ...
                        tb_parts.append(''.join(traceback.format_exception(
                            type(exception.__cause__), exception.__cause__, exception.__cause__.__traceback__)))
                    
                    error_log.traceback = ''.join(tb_parts)
            except Exception as tb_error:
                # 如果格式化堆栈跟踪失败，使用简单的字符串表示
                error_log.traceback = f"Exception: {str(exception)}\n无法获取完整堆栈跟踪: {str(tb_error)}"
        ...
        if request:
            ...
            # 打印调试信息    
            print(f"[log_system_error] 路径: {request.path}, 用户认证状态: {is_authenticated}, 用户ID: {user_id}")
            ...
            try:
                ...
                if request.content_type and ('application/json' in request.content_type or 
                    ...
                    # 排除敏感字段
                    safe_post = dict(request.POST)
                    for key in ['password', 'token', 'auth', 'key', 'secret']:
                        if key in safe_post:
                            safe_post[key] = '[REDACTED]'
                    context_data['request']['post_data'] = safe_post
                
                error_log.context_data = json.dumps(context_data)
            except Exception as context_error:
                print(f"[log_system_error] 收集请求上下文时出错: {str(context_error)}")
        
        # 保存前打印信息
        print(f"[log_system_error] 即将保存错误日志，用户ID: {error_log.user_id}, 路径: {error_log.path}, 类型: {error_log.error_type}")
        
        # 保存日志
        error_log.save()
        
        # 保存后检查是否有变化
        print(f"[log_system_error] 已保存错误日志，ID: {error_log.id}, 用户ID: {error_log.user_id}, 路径: {error_log.path}, 类型: {error_log.error_type}")
        ...
        print(f"[log_system_error] 记录错误日志时发生异常: {str(e)}")
        logger.critical(f"无法记录系统错误: {str(e)}")
        import traceback
        logger.critical(traceback.format_exc())
        return False

def _infer_error_type_from_exception(exception):
    """根据异常推断错误类型"""
    if not exception:
        return 'BACKEND'
    ...
    if isinstance(exception, DatabaseError):
        ...
        return 'VALIDATION'
    elif str(exception).lower().find('timeout') >= 0:
        return 'TIMEOUT'
    elif any(auth_term in str(exception).lower() for auth_term in ['auth', 'login', 'password', 'unauthorized']):
        return 'AUTH'
    
    return 'BACKEND'

def _infer_error_level_from_type(error_type, exception=None):
    """根据错误类型推断错误级别"""
    if error_type == 'DATABASE':
        return 'CRITICAL'
    elif error_type in ['PERMISSION', 'AUTH', 'VALIDATION']:
        return 'WARNING'
    elif error_type == 'API':
        return 'INFO' if exception and 'not found' in str(exception).lower() else 'WARNING'
